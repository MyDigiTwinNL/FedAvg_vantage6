...
class DeepSurv(nn.Module):
    ...
    def _build_network(self):
        ...
        for i in range(len(self.dims)-1):
            ...
            # adds linear layer
            # layers.append(nn.Linear(self.dims[i], self.dims[i+1], dtype=float))
            layers.append(nn.Linear(self.dims[i], self.dims[i+1], dtype=float, bias=False))
            if i != len(self.dims)-2:
                if self.norm: # adds batchnormalize layer
                    # layers.append(nn.BatchNorm1d(self.dims[i+1]))
                    layers.append(nn.BatchNorm1d(self.dims[i+1], affine=False, track_running_stats=False))
